...
import { createCommentValidationSchema, updateCommentValidationSchema } from './comments.validation-schema';
...
commentRouter.post('/', validator(createCommentValidationSchema), async ctx => {
  const { postId } = ctx.params;
  const commentData = {
    ...ctx.request.body,
    postId,
...
commentRouter.put('/:id', validator(updateCommentValidationSchema), async ctx => {
  const { id } = ctx.params;
  ...
  if (comment.userId !== ctx.state.user.id) {
    ctx.status = 403;
    ctx.body = { error: { message: 'Access denied - you can only update your own comments' } };
    return;
  }
...
commentRouter.delete('/:id', async ctx => {
  ...
  if (comment.userId !== ctx.state.user.id && ctx.state.user.role !== 'admin') {
    ctx.status = 403;
    ctx.body = { error: { message: 'Access denied - you can only delete your own comments' } };
    return;
  }
